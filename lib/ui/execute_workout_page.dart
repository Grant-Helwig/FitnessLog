...
import 'workout_history/bloc/workout_history_bloc.dart';
...
class ExecuteWorkout extends StatefulWidget {
  ...
  const ExecuteWorkout(
      {Key? key, required this.workouts, this.routine, this.history})
      : super(key: key);

  @override
  State<ExecuteWorkout> createState() => _ExecuteWorkoutState(
      workouts: this.workouts, routine: this.routine, history: this.history);
}
...
class _ExecuteWorkoutState extends State<ExecuteWorkout> {
  ...
  late StopWatchTimer stopWatchTimer = StopWatchTimer(onChange: (value) {
  ...
  Widget executeWorkoutCard(Workout workout, int index) {
    ...
    TextEditingController timerController = TextEditingController(
        text: Utils().printDuration(
            const Duration())); //Duration().toString().substring(0, Duration().toString().indexOf('.')));
    ...
    if (history != null) {
      ...
      recentHistory = repo.mostRecentWorkoutHistoryByWorkout(workout.id);
    }
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            if (hasDefault) {
              ...
              timerController =
                  TextEditingController(text: snapshot.data!.duration);
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Visibility(
                          visible:
                              (workout.type == WorkoutType.strength.index ||
                        ...
                        child: Row(
                          ...
                          children: [
                            SizedBox(
                              ...
                              child: TextFormField(
                                ...
                                onTap: () async {
                                  ...
                                  Duration? curTimer = Utils().parseDuration(
                                      timerController
                                  ...
                                  if (curTimer != null) {
                                    ...
                                    //);
                                    log("current timer${timerController.text}");
                                    duration = await Utils()
                                        .selectDuration(context, curTimer);
                                  } else {
                                    // duration = await showDurationPicker(context: context,
                                    ...
                                    duration = await Utils().selectDuration(
                                        context,
                                  ...
                                  setState(() {
                                    timerController.text = Utils().printDuration(
                                        duration!); //duration.toString().substring(0, duration.toString().indexOf('.') + 3);
                                  });
                                },
                              ),
                            ),
                            IconButton(
                              onPressed: () {
                                timerController.text = Utils().printDuration(
                                    stopWatchDuration); //stopWatchDuration.toString().substring(0, stopWatchDuration.toString().indexOf('.') + 3);
                              },
                      ...
                      Visibility(
                        ...
                        child: TextFormField(
                          ...
                          validator: (value) {
                            ...
                            if (workout.type == WorkoutType.both.index) {
                              if (timerController.text ==
                                      Utils().printDuration(const Duration()) &&
                                  (repControllers.isEmpty &&
                                      weightControllers.isEmpty) &&
                                  caloriesController.text.isEmpty &&
                                  heartRateController.text.isEmpty) {
                        ...
                        child: TextField(
                          ...
                          onTap: () async {
                            var timeTemp = (await showTimePicker(
                                context: context,
                                initialTime:
                                    TimeOfDay.fromDateTime(myDateTime)));
                            if (timeTemp != null) {
                              myDateTime = DateTime(
                                  myDateTime.year,
                                  myDateTime.month,
                            ...
                            TextButton(
                              onPressed: () async {
                                if (formKeys[index].currentState!.validate()) {
                                  WorkoutHistory tempWorkoutHistory =
                                      WorkoutHistory();
                                  ...
                                  tempWorkoutHistory.duration =
                                      timerController.text;
                                  tempWorkoutHistory.distance = double.parse(
                                      distanceController.text.isEmpty
                                          ? "0"
                                          : distanceController.text);
                                  tempWorkoutHistory.calories = double.parse(
                                      caloriesController.text.isEmpty
                                          ? "0"
                                          : caloriesController.text);
                                  tempWorkoutHistory.heartRate = double.parse(
                                      heartRateController.text.isEmpty
                                          ? "0"
                                  ...
                                  if (history != null) {
                                    ...
                                    await repo.updateWorkoutHistory(
                                        tempWorkoutHistory);
                                    newHistory = history!;
                                    ...
                                    newHistory = await repo
                                        .saveWorkoutHistory(tempWorkoutHistory);
                                  }

                                  for (int i = 0;
                                      i < repControllers.length;
                                      i++) {
                                    if (repControllers[i].text.isNotEmpty ||
                                        weightControllers[i].text.isNotEmpty) {
                                      WorkoutSet temp = WorkoutSet();
                                      temp.reps = int.parse(
                                          repControllers[i].text.isEmpty
                                              ? "0"
                                              : repControllers[i].text);
                                      temp.weight = double.parse(
                                          weightControllers[i].text.isEmpty
                                              ? "0"
                                  ...
                                  setState(() {
                                    ...
                                    if (cardsCompleted
                                      ...
                                      // final workoutHistoryBloc =
                                      //     context.read<WorkoutHistoryBloc>();
                                      // workoutHistoryBloc.state.maybeWhen(
                                      //     loaded: (dateRange, routines,
                                      //         selectedRoutine, workoutHistory) {
                                      //       workoutHistoryBloc.add(
                                      //           WorkoutHistoryEvent.started(
                                      //         dateRange,
                                      //         workout,
                                      //       ));
                                      //     },
